...
- 카테고리는 `CATEGORIES = ['자유','질문','착샷','후기']` — **배열 순서가 곧 탭 순서**
- 개편 전 이름(OOTD·옷 정보 문의·픽업샵 후기·이 옷 어디꺼?·구매 후기)은 `LEGACY_CATEGORY`로 매핑.
  **글을 읽을 때 항상 `normCategory()`를 거칠 것** — 카운트·필터·라벨 세 군데 모두. 안 그러면 옛 글이 어느 탭에도 안 잡힘
- DB의 옛 이름 정리는 관리자용 `migrateCategories()` 버튼(마이페이지). 규칙에 관리자의 category 단독 수정 허용이 있어야 동작함
